...
(
    ...
    [Parameter(ParameterSetName = 'Members', Mandatory = $false, HelpMessage = 'Enter a file path for report output. Must be CSV')]
    [ValidatePattern( '\.csv$' )]
    [Alias('ReportPath')]
    [String]$OutputPath,
    ...
    # Add optionally the Vault Context as prefix on generated logs ( to allow pattern search on logs when multiple running session ).
    [Parameter(Mandatory = $false)]
    [String]$VaultContext = '',
...
# Script Version
$ScriptVersion = '2.2.6+KVS_CUSTOM_20260225'

# ------ SET global parameters ------
# Set Log file path
$global:LOG_DATE = $(Get-Date -Format yyyyMMdd) + '-' + $(Get-Date -Format HHmmssffff)
$global:VAULT_CONTEXT = $VaultContext
if ([string]::IsNullOrEmpty($VAULT_CONTEXT)) {
    $global:LOG_FILE_PATH = "$ScriptLocation\..\log\SafeManagement_$LOG_DATE.log"
} else {
    $global:LOG_FILE_PATH = "$ScriptLocation\..\log\!_$VAULT_CONTEXT`_!_SafeManagement_$LOG_DATE.log"
}
...
#>
    ...
    try {
        ...
        $accSafeMembersURL += '?filter=includePredefinedUsers eq true&limit=50' 
    ...
    'Report' {
        ...
        try {
            ...
            if ([string]::IsNullOrEmpty($OutputPath)) {
                ...
                $output | Select-Object -Property safeName, managingCPM, numberOfVersionsRetention, numberOfDaysRetention, EnableOLAC | ConvertTo-Csv -NoTypeInformation | Out-File $OutputPath
            }
        }
        catch {
    ...
    { ($_ -eq 'Add') -or ($_ -eq 'AddMembers') -or ($_ -eq 'Update') -or ($_ -eq 'UpdateMembers') -or ($_ -eq 'Delete') -or ($_ -eq 'DeleteMembers') } {
        ...
        try {
            if (![string]::IsNullOrEmpty($FilePath)) {
                ...
                Write-LogMessage -type Verbose -MSG "Base:`tFilePath found and imported as CSV successfully"
                # Sort List by Safes
    ...
    'Members' {
        ...
        try {
            if ([string]::IsNullOrEmpty($UserName)) {
                ...
                $output = Get-SafeMembers -SafeName $SafeName
                if ([string]::IsNullOrEmpty($OutputPath)) {
                    $output
                }
                else {
                    $output | Select-Object -Property safeName, memberName, memberType, permissions | ConvertTo-Csv -NoTypeInformation | Out-File $OutputPath -Append
                }
            }
            else {
